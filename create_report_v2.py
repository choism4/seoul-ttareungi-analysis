...
# V21: compute_stats_v21.py의 Bai-Perron supF 재계산 결과로 업데이트
# supF = 54.61 @ 2021-09 (Andrews 1993 crit 8.85 초과 · H₀ 기각)
# 2021-09는 월 인덱스 (2021-2019)*12 + (9-1) = 32
r_break_idx = 32  # 2019.01=0 기준 2021.09=32
...
ax.axvspan(r_break_idx, len(residuals_plot), alpha=0.12, color='red',
           label='2021-09 분기점 이후 구간')
ax.axvline(x=r_break_idx, color='#8B0000', linestyle='--', linewidth=1.6, alpha=0.9)
ax.set_title('기온 효과 제거 후 잔차 시계열 — 2021년 9월 supF 분기점(F=54.61) 이후 구조적 이탈',
...
<tr><td>공간 자기상관</td><td>Queen I = +0.279 (p = 0.015) · LISA FDR 후 0 유의 · Moran power 0.33 @ n=25</td><td>전역 군집은 확인 · 국지 cluster는 n=25 검정력 한계로 500m hex 후속.</td></tr>
...
적절하다면 잔차는 0을 중심으로 무작위로 분포해야 한다. 그러나 supF가 특정한 2021년 9월
분기점(빨간 점선) 이후의 구간에서는 잔차가 지속적으로 0 아래로 이탈한다.</p>

<div class="chart">
<img src="data:image/png;base64,{charts['residual']}">
<div class="caption">[그림 4] 기온 효과 제거 후 잔차 시계열 — 2021년 9월 supF 분기점(F=54.61, 빨간 점선) 이후 구간에 음의 잔차 집중.</div>
</div>

<div class="killer">
<strong>Q1 검증 결과 — H₀ 기각, H₁ 지지:</strong> supF가 특정한 2021년 9월 분기점 이후 평균 잔차는
...
<p style="font-size:9.5pt; color:#555; margin:4px 0;">
전체 25자치구의 국지 Moran's I_i · raw p · BH-FDR 보정 p · 사분면을 수록한다.
</p>
...
결과로 해석된다. 4.2절에서 supF 검정이 가리킨 구조변화 시점이 <strong>2021년 9월</strong>로
...
※ <strong>HEAT 참여율 1% 가정의 보수성 근거:</strong> 서울시 자전거 수단분담률은 2.7%(2024)
수준이며, 이 중 따릉이를 규칙적으로 이용하는 (주 3회 이상, HEAT 정의) 비율을 40% 보수
적용하면 이용자 기준 참여율 ≈ 1% 수준이다. KDI PIMAC 건강편익 가이드의 권장 범위
0.5~2.5% 내에서 중하위값을 선택하여 편익 과대 추정을 방지한다.
<br>※ <strong>SROI 이중계상 방지:</strong> 탄소(CO₂ 감축) · 건강(신체활동) · 혼잡(시간가치)
세 편익은 각각 독립 경로를 통해 발생한다. HEAT 건강편익은 신체활동의 사망률 감소만
포함하며 대기오염 완화의 건강효과는 탄소 감축 편익에 귀속시켜 중복 회피한다. 혼잡 감소
편익은 승용차 대체분에만 적용되어 따릉이 내부 전환은 제외된다.
</p>
...
<tr><td>supF</td><td>sup-F 구조변화 검정 통계량</td><td>4.2.1 분기점 특정 (F=54.61 @ 2021-09)</td></tr>
...
<tr><td>F-통계량</td><td>구조변화 검정 통계량</td><td>supF = 54.61 @ 2021-09</td></tr>
</table>
