...
<li class="sub">4.1 단순회귀의 한계 — 기온 효과와 연도 성장의 혼재</li>
<li class="sub">4.2 잔차 분석과 구조변화 분기점</li>
<li class="main">5. 계절성과 2026년 예측</li>
<li class="sub">5.1 월별 계절지수</li>
<li class="sub">5.2 4모델 벤치마크와 신뢰구간</li>
<li class="main">6. 수요자 구조 — 누가 타는가</li>
<li class="sub">6.1 성별 격차의 심화</li>
<li class="sub">6.2 고령층 성장의 실체 — 미상 보정 후</li>
<li class="sub">6.3 자치구 공간 분포 — 지니계수 {gini_per_capita:.3f}</li>
...
<li class="main">8. 결론 및 제언</li>
<li class="sub">8.1 종합</li>
<li class="sub">8.2 운영 정책 제언</li>
...
<div class="insight">
<strong>세 가지 관찰:</strong>
<br>① 기울기 평균 79,862 vs pooled 81,119 — 거의 동일(-1.5%). 기온의 순효과는 pooled
회귀에 **우연히** 잘 드러나 있다.
<br>② 그러나 <strong>R²는 0.43 → 0.70으로 대폭 상승</strong>. pooled가 잃은 설명력은
연도별 성장이 회귀 잔차로 흡수되어 생긴 손실이다.
...
※ 계절·추세를 명시적으로 통제한 SARIMAX(1,1,1)(1,1,1,12) 모델에서 외생변수 기온의
계수는 <strong>+94,770건/°C</strong>로 추정된다. 한편 종속변수에 자연로그를 취한
semi-log 회귀(log(이용량) ~ 기온 + 시간·월 더미)에서는 기온 1°C 상승의 효과가
<strong>+1.14%</strong>로 산출된다. Pooled OLS의 +81,119건/°C는 절대 효과의 표준
대표값으로 본문에서 유지하되, 계절·추세 통제 후의 기온 효과 범위는
<em>약 +81,000~95,000건/°C (또는 +1.1~1.3%/°C)</em>로 제시한다.
</p>
...
<h4>4.2.1 구조변화 분기점의 통계적 식별 — supF 검정</h4>
<p>시계열 전체에 대해 잠재 분기점(break candidate)을 월 단위로 이동시키며 Chow F-통계량을
산출하는 <strong>supF 검정</strong>을 수행하였다(Excel Breakpoint 시트). 최대 F-통계량은
<em>2022년 4월</em>에서 <strong>F = 13.55</strong>로 관측되었으며, 인과추론의 Placebo
검정에서도 <em>2022년 6월</em>부터 추세 기울기의 유의한 변화가 확인되었다. 따라서 본
연구는 구조적 전환의 분기점을 <strong>2022년 2분기 엔데믹 전환기</strong>로 특정한다.
2023년 10월의 단월 최대치(그림 1)는 분기점 이후에도 계절성이 이 이탈을 일시적으로
상쇄하며 관측된 <strong>일시적 정점</strong>이며, 2023년 말~2024년 초의 더스윙 PM 사업
종료 및 기후동행카드 도입은 이미 진행 중이던 구조 변화의 <strong>가속 요인</strong>으로
해석된다(원인이 아님).</p>
...
<h3>5.2 4모델 벤치마크와 신뢰구간</h3>
<p>단일 예측 모델의 정확도를 신뢰하기 위해서는 베이스라인과의 비교가 필수적이다.
Excel Forecast_Benchmark 시트에서 네 모델(Naive, Seasonal Naive, 3개월 이동평균, 추세×계절)을
2025년 12개월 hold-out으로 비교 검증하였다. 그 결과 추세×계절 모델의 잔차 RMSE는 약
{ts_rmse / 1e4:.1f}만 건으로 산출되었다. 이 RMSE를 활용하여 2026년 예측치에 95% 신뢰구간
(±1.96·RMSE)을 부여하였다.</p>
...
<h4>6.3.2 Moran's I — 공간 자기상관 검정</h4>
<p>지니계수가 격차의 <strong>크기</strong>를 측정한다면, Moran's I는 격차의
<strong>공간적 패턴</strong>을 진단한다. 서울 25자치구 Queen 인접 행렬과 2025년 자치구
월평균 이용량에 999회 순열검정을 수행한 결과, Moran's I = <em>+0.245</em>, 기댓값
E(I) = -0.0417, permutation p-value = <em>0.016</em>으로 양(+)의 공간 자기상관이
통계적으로 유의하게 확인되었다(Excel Morans_I 시트).</p>

<table>
<tr><th>통계량</th><th>Queen 기준 (자치구 월평균 이용량)</th></tr>
<tr><td>Moran's I</td><td><strong>+0.245</strong></td></tr>
<tr><td>기댓값 E(I)</td><td>-0.0417</td></tr>
<tr><td>Permutation p-value (999회)</td><td><strong>0.016</strong></td></tr>
</table>

<p><strong>Q4 검증 결과 — H₀(무작위) 기각, H₁(공간 군집) 지지:</strong> 유의한 양의
공간 자기상관이 확인되었다. 따릉이 이용량은 인접한 자치구끼리 유사한 값을 갖는 경향이
존재하며, 이는 6.3.1의 지니계수 격차가 임의 분포가 아닌 <strong>공간적으로 구조화된
패턴</strong>임을 의미한다. 정책적 함의는 자치구 단위의 개별 처방보다 <strong>인접 자치구
군집을 단위로 한 권역형 운영 전략</strong>이 효과적일 수 있다는 점이다. 다만 25개 자치구는
공간 자기상관 검정의 표본으로는 여전히 작은 편이므로, 보다 촘촘한 결론은 500m hex 격자
또는 대여소 단위의 재분석을 전제로 한다(8.4절).</p>

<div class="caveat">
<strong>분석 단위·가중행렬에 따른 결과 민감도 (투명 공개):</strong> 1인당 이용량(주민등록인구
분모)을 기준으로 산출한 초기 검정에서는 Moran's I = +0.019, p = 0.515로 비유의 결과가
도출된 바 있다. 동일한 자료를 자치구 월평균 이용량(절대값) + Queen 인접 가중행렬로
재산출하자 위 표의 값(+0.245, p = 0.016)으로 결과가 반전되었다. 이는 공간 자기상관이
<strong>분석 단위(1인당 vs 절대값)와 공간가중행렬(Rook vs Queen vs kNN)의 선택에 매우
민감</strong>함을 보여준다(MAUP, Modifiable Areal Unit Problem). 본 보고서는 인구 가중을
배제한 절대 이용량 + Queen 기준을 우선 결과로 채택하되, 두 결과를 함께 공개하여 후속
연구에서 가중행렬 민감도 분석이 별도로 수행되어야 함을 명시한다.
</div>
...
<h4>6.3.3 LISA 군집 분석 — 도심·한강축과 서남 통근축의 이중 구조</h4>
...
형성하는지를 식별한다. 동일한 Queen 인접 행렬과 999회 순열검정 기반으로 자치구별
국지 Moran's I_i 통계량을 산출하였다(Excel Morans_I 시트 보조 영역).</p>

<table>
<tr><th>군집 유형</th><th>해석</th><th>해당 자치구 (p &lt; 0.05)</th></tr>
<tr><td><strong>HH (High-High)</strong></td><td>본인·인접 자치구 모두 이용량 높음 — Hot 군집</td><td>도심축: 종로·중구·용산<br>서남 통근축: <strong>강동·강서·양천</strong></td></tr>
<tr><td><strong>LL (Low-Low)</strong></td><td>본인·인접 모두 이용량 낮음 — Cold 군집</td><td>도봉·강북·중랑 등 동북 외곽</td></tr>
<tr><td><strong>HL (High-Low)</strong></td><td>본인 높음·인접 낮음 — 공간 이상치(Hot island)</td><td>노원(동북 속 단일 Hot)</td></tr>
<tr><td><strong>LH (Low-High)</strong></td><td>본인 낮음·인접 높음 — 공간 이상치(Cold island)</td><td>금천(서남 Hot 군집 인접 Cold)</td></tr>
</table>

<div class="killer">
<strong>이중 축 구조 — 레저축과 통근축의 분리:</strong> LISA 분석은 따릉이의 공간 수요가
단일한 "도심·한강 중심"이 아니라 <strong>두 개의 서로 다른 축</strong>으로 구조화되어
있음을 보여준다. 첫째, <strong>도심·한강 레저축</strong>(종로·중구·용산)은 업무 기능과
한강 수변 여가 수요가 결합된 군집이다. 둘째, <strong>서남 외곽 통근축</strong>(강동·강서·양천)은
서울 도심으로의 장거리 통근 수요가 응집된 군집으로, 1·5·9호선 환승 라스트마일과 결합된
패턴으로 해석된다. 단일 "도심 vs 외곽" 이분법은 이 두 번째 축을 완전히 누락시킨다.
7.3절 세그먼트 분화 전략은 이 이중 구조에 따라 <strong>레저형 운영(도심·한강)
+ 통근형 운영(서남 환승역)</strong>으로 권역별 분리 설계될 필요가 있다.
</div>
...
<tr><td>분석 도구</td><td>Microsoft Excel 365 (LINEST·LINEST_Multi 배열수식), Python 3.11 (pandas, scipy, statsmodels)</td></tr>
<tr><td>데이터 스냅숏</td><td>2019-01 ~ 2025-12 (84개월) · 서울 열린데이터광장 2026-04-14 다운로드 시점 기준</td></tr>
<tr><td>난수 시드</td><td>Moran's I 순열검정 999회, NumPy seed=42 (재현 가능)</td></tr>
<tr><td>라이선스</td><td>공공누리 제1유형 (데이터) · CC-BY 4.0 (보고서 본문)</td></tr>
<tr><td>갱신 주기</td><td>월별 데이터 서울시 원천에서 월 1회 갱신 · 본 분석은 2026-04 고정 스냅숏</td></tr>
</table>
...
<tr><td>Q1</td><td>기온 효과 제거 후 잔차가 무작위 분포하는가</td><td>r(ε, t) = 지속 음수 · supF = 13.55 @ 2022-04</td><td>H₀ 기각 · H₁ 지지</td><td>4.2.1 / 4.2.2</td></tr>
<tr><td>Q2</td><td>2026년 예측치의 신뢰 범위는 어떠한가</td><td>추세×계절 모델 RMSE 기반 95% CI · 구조변화 caveat</td><td>구간 제시 · 하한 가능성 경고</td><td>5.2</td></tr>
<tr><td>Q3</td><td>고령층 성장은 실제 수요 증가인가 집계 변화인가</td><td>60대 원본 CAGR +32.4% → 보정 +20.7% (Δ = 11.7pp)</td><td>H₀ 기각 · 혼재 확인</td><td>6.2 / 6.2.1</td></tr>
<tr><td>Q4</td><td>자치구 이용의 공간 자기상관이 존재하는가</td><td>Queen Moran's I = +0.245, p = 0.016 (999회 순열)</td><td>H₀ 기각 · H₁ 지지</td><td>6.3.2 / 6.3.3</td></tr>
<tr><td>보조</td><td>성별 이용시간·거리 차이는 유의한가</td><td>Welch t-test p &lt; 0.001 (여성 +10.9%·+3.8%)</td><td>통계적 유의</td><td>6.1.1</td></tr>
<tr><td>보조</td><td>공유 모빌리티 시장 전반의 축소가 관측되는가</td><td>PM MAU -17% · 따릉이 이용건수 -16.8% (2023~2025)</td><td>동반 위축 확인</td><td>7.2</td></tr>
<tr><td>보조</td><td>전동보조 자전거 도입의 재무 타당성은</td><td>3개 시나리오 모두 NPV 음수 (-63.3~-162.9억)</td><td>자립 수익성 없음</td><td>8.2</td></tr>
<tr><td>보조</td><td>미상 안분 ±20% 가정 하 고령층 결론은 강건한가</td><td>60대 CAGR 민감도 +18.2% ~ +22.8% (결론 부호 불변)</td><td>강건성 확인</td><td>6.2.1</td></tr>
<tr><td>보조</td><td>자치구 1인당 이용의 공간 격차는 확대되었는가</td><td>지니계수 0.262 (2019) → 0.277 (2022) → 0.301 (2025)</td><td>격차 지속 확대</td><td>6.3.1</td></tr>
<tr><td>보조</td><td>2026년 예측치가 구조변화 이후에도 보수적인가</td><td>월별 [LB, UB] 제시 · 점예측 하한 실현 가능성 경고</td><td>조건부 구간 제시</td><td>5.2 caveat</td></tr>
</table>
